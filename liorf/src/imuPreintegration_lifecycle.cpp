...
class TransformFusion : public ParamServerLifecycle
{
public:
    std::mutex mtx;
    ...
    TransformFusion(const rclcpp::NodeOptions &options)
        : ParamServerLifecycle("liorf_transformFusion", options)
    {
    }
        ...
        rclcpp_lifecycle::State const &state)
    {
        ...
        if(lidarFrame != baselinkFrame)
        {
            try
            {
                tf2::fromMsg(tfBuffer->lookupTransform(lidarFrame, baselinkFrame, rclcpp::Time(0)),
                             lidar2Baselink);
            }
    ...
    {
        // Publishers
        pubImuOdometry = create_publisher<nav_msgs::msg::Odometry>(
            odomTopic, QosPolicy(history_policy, reliability_policy));
        pubImuPath = create_publisher<nav_msgs::msg::Path>(
            "imu/path", QosPolicy(history_policy, reliability_policy));
        ...
        rclcpp_lifecycle::State const &state)
    {
        ...
        subImuOdometry.reset();
        pubImuOdometry.reset();
        pubImuPath.reset();
    ...
    Eigen::Affine3f odom2affine(nav_msgs::msg::Odometry odom)
    {
        ...
        tf2::Quaternion orientation(odom.pose.pose.orientation.x, odom.pose.pose.orientation.y,
    ...
    void imuOdometryHandler(const nav_msgs::msg::Odometry::SharedPtr odomMsg)
    {
        ...
        tf2::Transform tCur(
            ...
            tf2::Vector3(laserOdometry.pose.pose.position.x, laserOdometry.pose.pose.position.y,
        ...
        if(imuTime - last_path_time > 0.1)
        {
            ...
            while(!imuPath.poses.empty() &&
                  ROS_TIME(imuPath.poses.front().header.stamp) < lidarOdomTime - 1.0)
...
class IMUPreintegration : public ParamServerLifecycle
{
public:
    std::mutex mtx;
    ...
    IMUPreintegration(const rclcpp::NodeOptions &options)
        : ParamServerLifecycle("liorf_imuPreintegration", options)
    {
    }
        ...
        rclcpp_lifecycle::State const &state)
    {
        ...
        // tramsform points from lidar frame to imu frame
        ...
        preIntegrationParams_ = gtsam::PreintegrationParams::MakeSharedU(imuGravity);
        preIntegrationParams_->accelerometerCovariance =
            gtsam::Matrix33::Identity(3, 3) * pow(imuAccNoise, 2); // acc white noise in continuous
        preIntegrationParams_->gyroscopeCovariance =
            gtsam::Matrix33::Identity(3, 3) * pow(imuGyrNoise, 2); // gyro white noise in continuous
        preIntegrationParams_->integrationCovariance =
        ...
        imuIntegratorImu_ = std::make_unique<gtsam::PreintegratedImuMeasurements>(
    ...
    void odometryHandler(const nav_msgs::msg::Odometry::SharedPtr odomMsg)
    {
        ...
        while(!imuQueImu.empty() &&
              ROS_TIME((&imuQueImu.front())->header.stamp) < currentCorrectionTime - delta_t)
        {
...
int main(int argc, char **argv)
{
    ...
    e.add_node(ImuP->get_node_base_interface());
    e.add_node(TF->get_node_base_interface());
