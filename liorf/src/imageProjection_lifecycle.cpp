...
POINT_CLOUD_REGISTER_POINT_STRUCT(
    MulranPointXYZIRT,
    (float, x, x)(float, y, y)(float, z, z)(float, intensity, intensity)(uint32_t, t, t)(int,
                                                                                         ring,
                                                                                         ring))
...
class ImageProjection : public ParamServerLifecycle
{
private:
    std::mutex imuLock;
    std::mutex odoLock;
...
public:
    ImageProjection(const rclcpp::NodeOptions &options)
        : ParamServerLifecycle("liorf_imageProjection", options), deskewFlag(0)
    {
    }
    ...
    {
        imuTime.resize(queueLength);
        imuRotX.resize(queueLength);
        imuRotY.resize(queueLength);
        imuRotZ.resize(queueLength);
        ...
        rclcpp_lifecycle::State const &state)
    {
        ...
        pubExtractedCloud = create_publisher<sensor_msgs::msg::PointCloud2>(
            "deskew/cloud_deskewed", QosPolicy(history_policy, reliability_policy));
        ...
        rclcpp_lifecycle::State const &state)
    {
        ...
        subOdom.reset();
        subLaserCloud.reset();
        pubExtractedCloud.reset();
        pubLaserCloudInfo.reset();
    ...
    bool deskewInfo()
    {
        ...
        if(imuQueue.empty() || ROS_TIME(imuQueue.front().header.stamp) > timeScanCur ||
    ...
    void publishClouds()
    {
        ...
        cloudInfo.cloud_deskewed =
            publishCloud(pubExtractedCloud, fullCloud, cloudHeader.stamp, lidarFrame);
...
int main(int argc, char **argv)
{
    ...
    exec.add_node(IP->get_node_base_interface());
